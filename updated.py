...
for i, interval in enumerate(exercise_intervals):
    ...
    for joint in range(dictionary3D['E0_P0_T0_C1'].shape[1]):
        ...
        print("joint [", joint, "] entropy(x,y,z):[", calculate_entropy(x_axis), calculate_entropy(y_axis), calculate_entropy(z_axis), "]")
        print("joint [", joint, "] sRMS(x,y,z):[", calculate_sRMS(x_axis), calculate_sRMS(y_axis), calculate_sRMS(z_axis), "]")
